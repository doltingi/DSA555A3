...
public:
    ...
    /**
        Attempts to find the node associated with a given word segment.
        A word segment is found by descending character by character into the
        word node tree from the root.
        If the word segment is not found in the tree, nullptr is returned.
        If the word segment is found, a pointer to it is returned, regardless
        of whether it is a whole word
    */
    virtual iWordNode* getNodeForWord(const char* wordSegment) = 0;
